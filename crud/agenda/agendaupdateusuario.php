...
$id=$_SESSION['id'];
  ...
  echo'<div class="modal-dialog">';
     echo'<div class="modal-content">';
      echo'<form action="updateconsultausuario.php" method="POST">';
       ...
       echo'</div>';
        ...
        echo'<div class="form-group">';
         ...
         echo'<input type="date" class="form-control" name="data" value="'.$data.'" disabled >';
          ...
          $sqlusuario='SELECT * FROM pet p inner join usuario u on p.id_usuario=u.id';
          foreach ($dbh->query($sqlusuario)as $row) {
            }
          
          echo'<div class="form-group " >';
            echo'<label>CPF do Usuario </label>';
          echo'<input type="text" class="form-control" name="cpf" value="'.$row['cpf'].'" disabled>';
            echo'</div>';


            $id = $_SESSION['id'];

            $sqlpet="SELECT * FROM pet where id_usuario=$id ";
          ...
          echo' <select class="form-control" id="id_pet" name="id_pet">';
            ...
            foreach($dbh->query($sqlpet)as $row){
               $id_pet = $row['id_pet'];
                      echo'<option    
                      value="'.$row['id_pet'].'">
                      "'.$row['nomepet'].'" / Nome do Responsavel : '.$_SESSION['nome'].' </option>';
              }
      ...
      echo' <div class="control-group">
       ...
       echo'<button class="btn btn-default"><a href="agendarusuario.php">Cancelar</a></button>';
